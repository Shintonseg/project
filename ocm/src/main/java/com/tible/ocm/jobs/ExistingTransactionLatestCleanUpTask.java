...
@Component
@Slf4j
public class ExistingTransactionLatestCleanUpTask extends CommonTask<BaseTask, BaseTaskParameter> {
    ...
    public ExistingTransactionLatestCleanUpTask(BaseTaskService<BaseTask, BaseTaskParameter> taskService,
    ...
    @RabbitListener(bindings = {@QueueBinding(
            value = @Queue(value = "ExistingTransactionLatestCleanUpTask"),
            key = "task.ExistingTransactionLatestCleanUpTask",
    ...
    @Async
    @Scheduled(cron = "${tasks.existing-transactions-latest-cleanup-task}")
    @Override
    ...
    protected boolean toExecute(BaseTask task) {
        try {
            existingTransactionLatestService.deleteByPeriod(deleteOlderThan);
        } catch (Exception e) {
            log.error("Error occurred while try to clean up ExistingTransactionLatest", e);
            return false;
        }
        return true;
    }
    ...
    public String getTaskName() {
        return OcmTaskType.ExistingTransactionLatestCleanUpTask;
    }
}